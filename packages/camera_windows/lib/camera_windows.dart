...
/// One raw preview frame returned by [CameraWindows.getPreviewFrame].
///
/// The pixel data is in MFVideoFormat_RGB32 byte order (B, G, R, X),
/// top-down and not mirrored (the Windows preview texture itself is
/// shown mirrored).
class WindowsPreviewFrame {
  /// Creates a preview frame description.
  const WindowsPreviewFrame({
    required this.bytes,
    required this.width,
    required this.height,
  });

  /// Tightly packed BGRX pixel data (4 bytes per pixel).
...
class CameraWindows extends CameraPlatform {
  ...
  /// Channel for polling raw preview frames (fork addition).
  static const MethodChannel _previewFrameChannel = MethodChannel(
    'camera_windows/preview_frames',
  );

  /// Returns the most recent preview frame of [cameraId] without going
  /// through a still capture, or null when no frame is available yet.
  ///
  /// The first call enables frame caching on the native side, so it
  /// typically returns null until the next preview sample has arrived;
  /// callers are expected to poll.
  static Future<WindowsPreviewFrame?> getPreviewFrame(int cameraId) async {
    final Map<dynamic, dynamic>? reply = await _previewFrameChannel
    ...
    return WindowsPreviewFrame(
      bytes: reply['data']! as Uint8List,
