...
class CaptureController {
 public:
  ...
  // Copies the most recent preview frame (MFVideoFormat_RGB32, top-down,
  // not mirrored) into |data| and its dimensions into |width| / |height|.
  //
  // The first call enables frame caching, so it may return false until
  // the next preview sample arrives. Returns false while no frame is
  // available.
  virtual bool GetLatestPreviewFrame(std::vector<uint8_t>* data,
...
class CaptureControllerImpl : public CaptureController,
 ...
 public:
  ...
  bool GetLatestPreviewFrame(std::vector<uint8_t>* data, uint32_t* width,
 ...
 private:
  ...
  // Latest preview frame, cached by |UpdateBuffer| (called on a Media
  // Foundation callback thread) and read by |GetLatestPreviewFrame|
  // (called on the platform thread), so all access is guarded by
  // |latest_preview_frame_mutex_|. Caching starts on the first
  // |GetLatestPreviewFrame| call to keep the extra per-sample copy out
  // of plain preview use.
  std::mutex latest_preview_frame_mutex_;
  std::vector<uint8_t> latest_preview_frame_;
  uint32_t latest_preview_frame_width_ = 0;
  uint32_t latest_preview_frame_height_ = 0;
  bool latest_preview_frame_enabled_ = false;
