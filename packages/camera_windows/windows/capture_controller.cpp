...
bool CaptureControllerImpl::UpdateBuffer(uint8_t* buffer,
  ...
  {
    const std::lock_guard<std::mutex> lock(latest_preview_frame_mutex_);
    if (latest_preview_frame_enabled_ && preview_frame_width_ > 0 &&
        preview_frame_height_ > 0) {
      latest_preview_frame_.assign(buffer, buffer + data_length);
      latest_preview_frame_width_ = preview_frame_width_;
      latest_preview_frame_height_ = preview_frame_height_;
    }
  }
  return texture_handler_->UpdateBuffer(buffer, data_length);
}

bool CaptureControllerImpl::GetLatestPreviewFrame(std::vector<uint8_t>* data,
                                                  uint32_t* width,
                                                  uint32_t* height) {
  const std::lock_guard<std::mutex> lock(latest_preview_frame_mutex_);
  // The first call only enables caching; a frame arrives with the next
  // preview sample.
  latest_preview_frame_enabled_ = true;
  if (latest_preview_frame_.empty()) {
    return false;
  }
  *data = latest_preview_frame_;
  *width = latest_preview_frame_width_;
  *height = latest_preview_frame_height_;
  return true;
}
