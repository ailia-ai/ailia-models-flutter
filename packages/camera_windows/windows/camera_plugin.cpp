...
void CameraPlugin::RegisterWithRegistrar(
  ...
  // Fork addition: channel for polling raw preview frames. Handled on
  // the platform thread, so reading the cached frame is safe (unlike
  // pushing frames from the Media Foundation callback thread, which is
  // why the upstream streaming implementation was reverted).
  plugin->preview_frame_channel_ =
      std::make_unique<flutter::MethodChannel<EncodableValue>>(
          registrar->messenger(), "camera_windows/preview_frames",
          &flutter::StandardMethodCodec::GetInstance());
  CameraPlugin* plugin_pointer = plugin.get();
  plugin->preview_frame_channel_->SetMethodCallHandler(
      [plugin_pointer](const auto& call, auto result) {
        plugin_pointer->HandlePreviewFrameMethodCall(call, std::move(result));
      });

  registrar->AddPlugin(std::move(plugin));
}

void CameraPlugin::HandlePreviewFrameMethodCall(
    const flutter::MethodCall<EncodableValue>& call,
    std::unique_ptr<flutter::MethodResult<EncodableValue>> result) {
  if (call.method_name() != "getPreviewFrame") {
    result->NotImplemented();
    return;
  }
  const auto* arguments = std::get_if<EncodableMap>(call.arguments());
  if (!arguments) {
    result->Error("argument_error", "Expected a cameraId argument map");
    return;
  }
  auto camera_id_it = arguments->find(EncodableValue("cameraId"));
  if (camera_id_it == arguments->end()) {
    result->Error("argument_error", "Missing cameraId");
    return;
  }
  const int64_t camera_id = camera_id_it->second.LongValue();

  Camera* camera = GetCameraByCameraId(camera_id);
  if (!camera) {
    result->Error("camera_error", "Camera not created");
    return;
  }
  CaptureController* capture_controller = camera->GetCaptureController();
  if (!capture_controller) {
    result->Error("camera_error", "Camera not initialized");
    return;
  }
  ...
  uint32_t width = 0;
  uint32_t height = 0;
  if (!capture_controller->GetLatestPreviewFrame(&data, &width, &height)) {
    // No frame available yet; the caller polls again.
    result->Success();
    return;
  }

  EncodableMap reply;
  reply[EncodableValue("data")] = EncodableValue(std::move(data));
  reply[EncodableValue("width")] = EncodableValue(static_cast<int64_t>(width));
  reply[EncodableValue("height")] =
